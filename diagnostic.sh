...
case "$OPTION" in
	...
	(echo >/dev/tcp/rhui-1.microsoft.com/443) && echo "Open 443" || echo "Close 443" >> /tmp/diagnostic 2>&1
	(echo >/dev/tcp/rhui-2.microsoft.com/443) && echo "Open 443" || echo "Close 443" >> /tmp/diagnostic 2>&1
	(echo >/dev/tcp/rhui-3.microsoft.com/443) && echo "Open 443" || echo "Close 443" >> /tmp/diagnostic 2>&1
	for x in rhui-1.microsoft.com rhui-2.microsoft.com rhui-3.microsoft.com ; do echo -e "\n" && (echo >/dev/tcp/$x/443) && echo "Open 443" || echo "Close 443" | tee /tmp/diagnostic ; done
	cat /tmp/diagnostic
	;;
