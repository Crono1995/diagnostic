...
case "$OPTION" in
	...
	for x in rhui-1.microsoft.com rhui-2.microsoft.com telnet rhui-3.microsoft.com ; do (echo >/dev/tcp/$x/443) &>/dev/null && echo "$x open" || echo "$x close" >> /tmp/diagnostic ; done
	cat /tmp/diagnostic
	;;
